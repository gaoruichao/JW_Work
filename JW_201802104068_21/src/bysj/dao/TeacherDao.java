...
public final class TeacherDao {
	...
	public Collection<Teacher> findAll(){
		...
		try{
			...
			while (resultSet.next()) {
				...
				System.out.print(resultSet.getString("profTitle"));
				System.out.print(",");
				//打印结果集中记录的degree字段
	...
	public boolean add(Teacher teacher) throws SQLException,ClassNotFoundException{
		...
		Connection connection = JdbcHelper.getConn();
		//写sql语句
		String addTeacher_sql = "INSERT INTO teacher (name) VALUES"+" (?)";
		//在该连接上创建预编译语句对象
	...
	public boolean update(Teacher teacher) throws ClassNotFoundException,SQLException{
		...
		Connection connection = JdbcHelper.getConn();
		//写sql语句
		String updateDegree_sql = " update teacher set name=?,profTitle=?,degree=?,department=? where id=?";
		//在该连接上创建预编译语句对象
